      ...
      if (res) {
        ...
        if (res.data.errors) {
          if (
            ...
            notify("اسم المستخدم موجود مسبقا", "error");
          }
        }
        if (res.data.errors && res.data.errors.length > 0) {
          ...
          const errorMsg = res.data.errors[0].msg; // الحصول على الرسالة من المصفوفة
          const message = errorMessages[errorMsg];

          if (message) {
            notify(message, "error");
          } else {
            notify("حدث خطأ غير معروف", "error"); // إشعار افتراضي إذا لم يكن الخطأ ضمن القائمة
          }
        }

        if (res.data.error) {
          if (res.data.errors[0].msg === "Email already exists") {
            notify("الايميل موجود مسبقا", "error");
          }
        }
      }
